...
class Deployer {
  ...
  async start() {
    ...
    this.log(`[KEY] ${this.key.publicKey}`)
    ...
    const stringAddress = walletAddress.toString(true, true, false)
  ...
  async work() {
    ...
    try {
      this.log(
        `[Deployer] collection address ${this.collection.address?.toString(true, true, false)}`
      )
